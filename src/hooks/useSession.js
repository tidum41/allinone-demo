...
export function useSession() {
  const [lines, setLines] = useState(() => ssGet('demo:v4:lines', DEMO_LINES))
  const [categoryRules, setCategoryRules] = useState(() => ssGet('demo:v4:rules', []))
  const [notes, setNotes] = useState(() => ssGet('demo:v4:notes', DEFAULT_NOTES))
  const [savedTasks, setSavedTasks] = useState(() => ssGet('demo:v4:tasks', []))
  const [savedCompleted, setSavedCompleted] = useState(() => ssGet('demo:v4:completed', []))
  const [prevSortedTasks, setPrevSortedTasks] = useState(() => ssGet('demo:v4:prevTasks', []))
  ...
  const updateLines = useCallback((newLines) => {
    setLines(newLines)
    ssSet('demo:v4:lines', newLines)
  }, [])

  const persistTasks = useCallback((tasks) => {
    setSavedTasks(tasks)
    ssSet('demo:v4:tasks', tasks)
  }, [])
  ...
  const persistCompleted = useCallback((task) => {
    ...
    setSavedCompleted(prev => {
      ...
      ssSet('demo:v4:completed', updated)
      return updated
    })
  }, [])
  ...
  const persistPrevTasks = useCallback((tasks) => {
    ...
    ssSet('demo:v4:prevTasks', tasks)
  }, [])

  const persistPreSortBlob = useCallback((lines) => {
    ssSet('demo:v4:preSortBlob', lines)
  }, [])
